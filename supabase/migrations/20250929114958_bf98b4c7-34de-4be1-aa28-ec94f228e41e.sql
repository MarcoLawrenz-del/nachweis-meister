-- Create project_sub and requirements for the newly created subcontractor
...
-- Generate requirements for all demo subcontractors that don't have any yet
DO $$
DECLARE
  sub_record RECORD;
...
BEGIN
  ...
  LOOP
    ...
    IF project_sub_uuid IS NOT NULL THEN
      ...
      RAISE NOTICE 'Generated requirements for subcontractor %: %', sub_record.id, result;
    END IF;
  END LOOP;
END $$;